	...
	if best >= 0:
		for pid in Galaxy.stars[best].planetIds:
			if int(Galaxy.planets[pid].population) > 0:
				...
				# The AI builds a station on each serviced planet (aiHasStation in
				# the original) so its trains can load/unload (cargo gating).
				Galaxy.planets[pid].hasStation = true
				Galaxy.planets[pid].aiHasStation = true
