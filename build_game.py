...
function _carUnitCost(type){
  if(type==='car_royal') return 10000;
...
// ── game speed ───────────────────────────────────────────────
// Index 0 = paused (multiplier 0), index 1 = 0.5×, index 2 = 1× (default),
// then 2×, 5×, 10×. Left/right arrow buttons (and ←/→ keys) walk the index.
...
function _genResortBiome(_R){
  _R=_R||Math.random;
  const _blobPts=(rx,ry,rot,n,rough)=>{
    const pts=[];
    for(let i=0;i<n;i++){
      const a=(i/n)*Math.PI*2+rot, p=1.0-rough*0.5+_R()*rough;
  ...
  const bigA=_R()*Math.PI*2, bigD=0.56+_R()*0.28;
  const bigRx=0.44+_R()*0.30, bigRy=0.24+_R()*0.22, bigRot=_R()*Math.PI;
  lms.push({cx:Math.cos(bigA)*bigD, cy:Math.sin(bigA)*bigD,
    ...
    sand:_rc((218+~~(_R()*24))+','+(194+~~(_R()*20))+','+(114+~~(_R()*30))),
    veg:_rc((44+~~(_R()*30))+','+(124+~~(_R()*34))+','+(40+~~(_R()*24))),
    deep:_rc((14+~~(_R()*16))+','+(64+~~(_R()*24))+','+(14+~~(_R()*18)))});
  // 6-12 small islands — fewer blob points, moderate roughness
  const n=6+~~(_R()*7);
  for(let i=0;i<n;i++){
    const a=_R()*Math.PI*2, d=_R()*0.84;
    const rx=0.024+_R()*0.068, ry=0.016+_R()*0.044, rot=_R()*Math.PI;
    lms.push({cx:Math.cos(a)*d, cy:Math.sin(a)*d,
      rx, ry, rot, isLarge:false,
      pts:_blobPts(rx,ry,rot,6,0.54),
      sand:'rgba('+(194+~~(_R()*36))+','+(168+~~(_R()*28))+','+(96+~~(_R()*36))+',0.92)',
      veg:'rgba('+(40+~~(_R()*36))+','+(108+~~(_R()*40))+','+(36+~~(_R()*28))+',0.89)',
      deep:null});
  }
  const sws=[];
  for(let i=0;i<2+~~(_R()*4);i++){
    const a=_R()*Math.PI*2, d=_R()*0.72;
    sws.push({cx:Math.cos(a)*d, cy:Math.sin(a)*d,
      rx:0.05+_R()*0.11, ry:0.04+_R()*0.08, rot:_R()*Math.PI});
  }
...
function _genAgriBiome(_R){
  _R=_R||Math.random;
  const fields=[];
  // 5-20 large clusters + 25-50 small clusters (5× original density)
  const nL=5+~~(_R()*16), nS=25+~~(_R()*26);
  const clusters=[];
  for(let i=0;i<nL;i++){
    const a=_R()*Math.PI*2, d=_R()*0.62;
    clusters.push({cx:Math.cos(a)*d, cy:Math.sin(a)*d, spread:0.13+_R()*0.10, n:8+~~(_R()*10)});
  }
  for(let i=0;i<nS;i++){
    const a=_R()*Math.PI*2, d=_R()*0.80;
    clusters.push({cx:Math.cos(a)*d, cy:Math.sin(a)*d, spread:0.04+_R()*0.06, n:3+~~(_R()*5)});
  }
  for(const cl of clusters){
    for(let fi=0;fi<cl.n;fi++){
      const fa=_R()*Math.PI*2, fd=_R()*cl.spread;
      const nx=cl.cx+Math.cos(fa)*fd, ny=cl.cy+Math.sin(fa)*fd;
      if(nx*nx+ny*ny>0.84) continue;
      fields.push({x:nx, y:ny,
        w:0.044+_R()*0.055, h:0.018+_R()*0.028,
        rot:_R()*Math.PI,
        col:_CROP_COLS[~~(_R()*_CROP_COLS.length)]});
    }
  }
  return {id:'agri', fields};
}
// Oil biome: generates random shimmer band data for iridescent surface rendering
function _genOilBiome(_R){
  _R=_R||Math.random;
  ...
  for(let i=0;i<N;i++){
    bands.push({
      angle: _R()*Math.PI*2,        // rotation angle of the band
      spread: 0.18+_R()*0.30,       // angular width (radians)
      phase: _R()*Math.PI*2,        // color wheel phase offset
      alpha: 0.30+_R()*0.28,       // max opacity of this band
    });
  }
  ...
  for(let i=0;i<G;i++){
    const a=_R()*Math.PI*2, d=_R()*0.72;
    glints.push({cx:Math.cos(a)*d, cy:Math.sin(a)*d, r:0.04+_R()*0.12, phase:_R()*Math.PI*2});
  }
  return {id:'oil', bands, glints};
}
// Storm planet: generate lightning bolt data for animated rendering
function _genStormLightning(_R){
  _R=_R||Math.random;
  // Local seeded versions of rand/randInt so the global helpers don't leak unseeded randomness in.
  const _rd=(a,b)=>_R()*(b-a)+a;
  const _ri=(a,b)=>Math.floor(_rd(a,b+0.999));
  const bolts=[];
  const N=3+_ri(0,4); // 3–6 radial bolts (originate near centre)
  for(let i=0;i<N;i++){
    ...
    const lat=_rd(-0.55,0.55); // starting latitude (sphere Y fraction)
    const angle=_R()*Math.PI*2; // horizontal angle of bolt start
    const nSeg=3+_ri(0,3); // 3–5 segments
    // Build zigzag path: each step moves "inward→outward" in radius
    const pts=[];
    let r0=_rd(0.08,0.35), a0=angle;
    pts.push({r:r0,a:a0,lat});
    for(let s=0;s<nSeg;s++){
      r0+=_rd(0.10,0.22);
      a0+=_rd(-0.55,0.55);
      const la=lat+_rd(-0.08,0.08);
      pts.push({r:Math.min(r0,0.98),a:a0,lat:la});
    }
    bolts.push({
      pts,
      phase:_R()*6000,      // timing offset in ms
      period:1800+_R()*3200, // ms between flashes
      dur:60+_R()*100,       // ms flash lasts
      br:_R()<0.3            // branch: secondary fork?
    });
  }
  ...
  const floatBolts=[];
  const M=2+_ri(0,4); // 2–5 atmospheric arc bolts
  for(let i=0;i<M;i++){
    const lat=_rd(-0.62,0.62);
    const angle=_R()*Math.PI*2;
    const nSeg=3+_ri(0,4); // 3–6 segments — can be longer arcs
    const pts=[];
    let r0=_rd(0.50,0.82), a0=angle;
    pts.push({r:r0,a:a0,lat});
    for(let s=0;s<nSeg;s++){
      r0+=_rd(-0.07,0.09); // slight drift in/out — stays mid-atmosphere
      r0=Math.max(0.35,Math.min(0.97,r0));
      a0+=_rd(-0.65,0.65); // large lateral sweep per segment
      const la=lat+_rd(-0.07,0.07);
...
function _attachBiome(p){
  // Assign a stable per-planet seed so reloads regenerate identical landmasses.
  // Existing saves without biomeSeed get one assigned at first restore, which
  // then persists in subsequent saves.
  if(p.biomeSeed==null) p.biomeSeed=(Math.floor(Math.random()*0xFFFFFFFE)+1)>>>0;
  const _R=_mkRng(p.biomeSeed);
  if(p.type.id==='resort') p.type={...p.type,_b:_genResortBiome(_R)};
  else if(p.type.id==='agri') p.type={...p.type,_b:_genAgriBiome(_R)};
  else if(p.type.id==='oil')  p.type={...p.type,_b:_genOilBiome(_R)};
  else if(p.type.id==='storm') p.type={...p.type,_b:_genStormLightning(_R)};
  // Mountains get a derived seed (XOR with golden ratio constant) so the
  // biome generator and mountain generator don't draw from the same stream.
  const _mtn=generatePlanetMountains(p, _mkRng((p.biomeSeed^0x9e3779b9)>>>0));
...
function generatePlanetMountains(p, _R){
  _R=_R||Math.random;
  const _rd=(a,b)=>_R()*(b-a)+a;
  const _ri=(a,b)=>Math.floor(_rd(a,b+0.999));
  const id=p.type.id;
  ...
  const mtns=[];
  for(let i=0;i<n;i++){
    const h=_rd(0.12,0.30);   // peak height as fraction of r beyond surface
    const tall=h>0.20;
    mtns.push({
      a:_R()*Math.PI*2,
      h,
      wHalf:_rd(0.05,0.14),   // half angular width in radians
      tipH:tall?h*_rd(0.28,0.48):0,
      col, tall
    });
  }
  return mtns;
...
function computeDemandRate(p){
  ...
  // Steel demand floor: a Large Station planet always has ≥1 steel demand rate
  // so the player can accumulate the 6 steelDelivered units required for the
  // Terminal upgrade. Mirror of the iron-for-Large-Station pattern above.
  if(p.hasLargeStation) r.steel=Math.max(r.steel||0, 1.0);
  ...
  if(t.cargoPhase==='unloading'){
    ...
    if(cargo&&p){
      if(t._hazmatStarDump){
        ...
        // Designing a better space train: count steel/battery/oil deliveries to the home planet
        if(cargo==='steel'||cargo==='battery'||cargo==='oil'){
          const _dbtMx=missions.find(mx=>mx.id==='design_better_train'&&mx.status==='active');
          if(t.isPlayer&&_dbtMx&&p.id===_dbtMx.targetPlanetId){
            if(cargo==='steel')   _dbtMx._homeSteelDelivered  =(_dbtMx._homeSteelDelivered  ||0)+_cUU;
            if(cargo==='battery') _dbtMx._homeBatteryDelivered=(_dbtMx._homeBatteryDelivered||0)+_cUU;
            if(cargo==='oil')     _dbtMx._homeOilDelivered    =(_dbtMx._homeOilDelivered    ||0)+_cUU;
          }
        }
    ...
    if((p.upgrades||[]).includes('blast_furnace')&&p.upgradeData?.blast_furnace){
      ...
      if((_bf.progress||0)>0){
        ...
        if(_bf.progress<=0){
          ...
          // N700 unlock check (200 units in any 20-stardate window).
          _steelProdLog.push({sd:stardate,units:1});
          if(!_steelCarUnlocked){
            ...
            if(_steelMissionTimerMs===0) _steelMissionTimerMs=Date.now();
          }
        }
...
function drawSpeedIndicator(){
  ...
  // Label + color depend on band: paused / slow / 1× / fast
  let _spdLbl, _spdCol;
  if(gameSpeedIdx===0){ _spdLbl='P';    _spdCol='rgba(200,210,225,0.85)'; }
  else if(spd<1)      { _spdLbl='0.5X'; _spdCol='rgba(180,225,255,0.85)'; }
...
function drawNewMissionPopup(){
  ...
  if(def.details){
    ...
    // Resolve {SOURCE}/{TARGET} placeholders before word-wrap.
    let _dtTxt=def.details;
    if(_dtTxt.includes('{SOURCE}')&&popupState.sourcePlanetId!=null){const _sp4=_gp(popupState.sourcePlanetId);if(_sp4)_dtTxt=_dtTxt.replace(/\{SOURCE\}/g,_sp4.name);}
    if(_dtTxt.includes('{TARGET}')&&popupState.targetPlanetId!=null){const _tp4=_gp(popupState.targetPlanetId);if(_tp4)_dtTxt=_dtTxt.replace(/\{TARGET\}/g,_tp4.name);}
    const _dtWords=_dtTxt.split(' ');
    let _dtLine='';
...
// Engine unlock policy (post-rebalance):
//   • Class J + Class R — unlocked by completing the "Designing a better space
//     train" mission (see mission-complete handler in updateMissions).
//   • N700 — unlocked the first time the player has produced ≥200 units of
...
function _checkEngineUnlocks(){
  ...
  let _recent=0; for(const e of _steelProdLog) _recent+=e.units;
  if(_recent>=200){
...
// Accumulator so updateMissions runs at most once per 0.1 SD instead of every
// rendered frame. The mission system's intro-gate checks and per-objective
// sweep scan galaxy.planets and trains[] — at mid/late game with many active
// missions this is the single biggest hot loop. All conditions advance from
// either delivery events (which fire immediately at the unload site) or
// slow-moving world state (planet visits, station counts), so a ~150ms
// detection lag at 1× speed is imperceptible.
let _missionTickAcc=0;
function updateMissions(dtSd){
  if(!galaxy) return;
  _missionTickAcc+=dtSd||0;
  if(_missionTickAcc<0.1) return;
  _missionTickAcc=0;
  ...
  // design_better_train intro: arm timer once first steel produced; fire 10 real-time seconds later.
  // The timer self-rearms on load (since it's not persisted) so it survives save/load cycles.
  if(_steelMissionTimerMs===0&&_steelCarUnlocked&&!_missionPending('design_better_train')){
...
function drawMissionsPopup(){
  ...
  const _mObjProgress=(m,objId)=>{
    ...
    if(objId==='deliver_10_steel_home')   return {cur:Math.min(m._homeSteelDelivered  ||0,20), max:20, label:'Steel delivered'};
    if(objId==='deliver_10_battery_home') return {cur:Math.min(m._homeBatteryDelivered||0,20), max:20, label:'Batteries delivered'};
    if(objId==='deliver_10_oil_home')     return {cur:Math.min(m._homeOilDelivered    ||0,20), max:20, label:'Oil delivered'};
    return null;
  };
  ...
  for(let mi=0;mi<_sorted.length;mi++){
    ...
    for(let _oi=0;_oi<m.objectives.length;_oi++){
      ...
      if(_prog){
        ...
        // Label sits a hair above the centre of the PROG_H block so the
        // bar visually balances between the objective text above and the
        // next objective's text below (previously the bar hugged the next
        // objective with a much smaller gap above than below).
        const _pLabelY=_mRowY+_objTotalH-3;
...
function drawSaveManagerPopup(){
  ...
  // Autosaves now split per corp (id starts with 'autosave_'). Show every
  // autosave at the top, most-recently-written first, then all manual saves.
  const _autosaveEntries=_idx.filter(e=>_isAutosaveId(e.id)).sort((a,b)=>b.savedAt-a.savedAt);
  const _manualEntries=_idx.filter(e=>!_isAutosaveId(e.id)).sort((a,b)=>b.savedAt-a.savedAt);
  const _list=[];
  for(const e of _autosaveEntries) _list.push(e);
  ...
  if(_list.length===0){
    ...
    for(let i=0;i<_list.length;i++){
      ...
      if(_ry+_rowH<_listY||_ry>_listY+_listH) continue;
      const _isAutosave=_isAutosaveId(e.id);
      ...
      const _title=_isAutosave?('AUTOSAVE — '+(e.corp||'(unknown corp)')):(e.label||(e.corp||'Save')+'_'+e.sd.toFixed(2).replace('.',''));
...
function _drawUpgradesPanel(mainPx,mainPy,mainPh,p){
  ...
  for(let i=0;i<shown.length;i++){
    ...
    if(u.isStation){
      ...
      drawPlanetStation(_mvx,_mvy,_mvr,Math.PI*1.2,20,p.isAlienRelic||false,p.hasLargeStation||false,null,p.hasTerminal||false);
      ctx.restore();
      if(isBuilt){
        if(p.hasTerminal){
      ...
      ctx.beginPath(); ctx.arc(_mvx,_mvy,_mvr,0,Math.PI*2); ctx.fill();
      drawFoundryBuilding(_mvx,_mvy,_mvr,Math.PI*0.75,20,(p.upgradeData?.iron_foundry?.progress||0)>0,'iron_foundry');
      ctx.restore();
      if(isBuilt){
      ...
      ctx.beginPath(); ctx.arc(_mvx,_mvy,_mvr,0,Math.PI*2); ctx.fill();
      drawFoundryBuilding(_mvx,_mvy,_mvr,Math.PI*0.75,20,(p.upgradeData?.blast_furnace?.progress||0)>0,'blast_furnace');
      ctx.restore();
      if(isBuilt){
        ...
        _bfBar('CHEMICAL',_bf.chemical||0,itemY+46,'rgba(96,216,32,0.78)');
        const _prog=_bf.progress||0;
        if(_prog>0){
          const _frac=1-_prog/BLAST_FURNACE_PROD_TIME;
          const _pbarY=itemY+62;
          ctx.font='bold 7px Orbitron,sans-serif'; ctx.textAlign='left';
          // Yellow production-progress palette — standardized across every
          // refinery (foundry / blast furnace / glassworks / factory /
          // bakery / juicery) so a "bar filling up to make a new resource"
          // reads as the same UI motif everywhere.
          ctx.fillStyle='rgba(255,200,60,0.85)'; ctx.fillText('BLASTING',_barX,_pbarY-1);
      ...
      ctx.beginPath(); ctx.arc(_mvx,_mvy,_mvr,0,Math.PI*2); ctx.fill();
      drawFoundryBuilding(_mvx,_mvy,_mvr,Math.PI*0.75,20,(p.upgradeData?.glassworks?.progress||0)>0,'glassworks');
      ctx.restore();
      if(isBuilt){
        ...
        _gwBar('CHEMICAL',_gw.chemical||0,itemY+46,'rgba(96,216,32,0.78)');
        const _prog=_gw.progress||0;
        if(_prog>0){
          const _frac=1-_prog/FOUNDRY_PROD_TIME;
          const _pbarY=itemY+62;
          ctx.font='bold 7px Orbitron,sans-serif'; ctx.textAlign='left';
          // Yellow production-progress palette (see _bfBar above).
          ctx.fillStyle='rgba(255,200,60,0.85)'; ctx.fillText('MELTING',_barX,_pbarY-1);
      ...
      ctx.beginPath(); ctx.arc(_mvx,_mvy,_mvr,0,Math.PI*2); ctx.fill();
      drawFoundryBuilding(_mvx,_mvy,_mvr,Math.PI*0.75,20,(p.upgradeData?.factory?.progress||0)>0,'factory');
      ctx.restore();
      if(isBuilt){
        ...
        if(_prog>0){
          ...
          ctx.font='bold 7px Orbitron,sans-serif'; ctx.textAlign='left';
          // Yellow production-progress palette (see _bfBar above).
          ctx.fillStyle='rgba(255,200,60,0.85)'; ctx.fillText('BUILDING',_barX,_pbarY-1);
      ...
      ctx.beginPath(); ctx.arc(_mvx,_mvy,_mvr,0,Math.PI*2); ctx.fill();
      drawFoundryBuilding(_mvx,_mvy,_mvr,Math.PI*0.75,20,(_bjData.progress||0)>0,_isBakery?'bakery':'juicery');
      ctx.restore();
      if(isBuilt){
        const _barX=upX+12, _barW=upW-28, _barH=6;
        ...
        if(_prog>0){
          ...
          ctx.font='bold 7px Orbitron,sans-serif'; ctx.textAlign='left';
          // Yellow production-progress palette (see _bfBar above).
          ctx.fillStyle='rgba(255,200,60,0.85)'; ctx.fillText(_bjVerb,_barX,_pbarY-1);
...
function drawPlanetDetailPopup(){
  ...
  if(activeTab==='station'){
    ...
    // Stats tab — planet activity grid above + top-5 train visits below
    popupState.buildStationBtnBounds=null;
    popupState.buildStationOverlayBounds=null;

    // ── Section 1: PLANET ACTIVITY stat grid ───────────────────
    ctx.font='bold 9px Orbitron,sans-serif'; ctx.textAlign='left';
    ctx.fillStyle='rgba(100,160,255,0.7)'; ctx.fillText('PLANET ACTIVITY',px+14,tcY+14);

    // Pre-compute aggregates from orbit counts + devLog + planet fields.
    const poc=planetOrbitCounts[p.id]||{};
    const pocE=Object.entries(poc).sort((a,b)=>b[1]-a[1]);
    const _totalOrb=pocE.reduce((s,e)=>s+(e[1]||0),0);
    const _uniqTrains=pocE.length;
    const _devLog=p.devLog||[];
    const _devPts=_devLog.reduce((s,e)=>s+(e.value||0),0);
    const _firstSd=_devLog.length?_devLog[0].sd:null;
    const _lastSd=_devLog.length?_devLog[_devLog.length-1].sd:null;
    // Top cargo: highest-count cargo type across all deliveries
    const _cargoCounts={};
    for(const _e of _devLog){ if(_e.cargo) _cargoCounts[_e.cargo]=(_cargoCounts[_e.cargo]||0)+1; }
    const _topCargoPair=Object.entries(_cargoCounts).sort((a,b)=>b[1]-a[1])[0];
    // Last cargo: most recent entry's cargo type
    const _lastCargoType=(()=>{ for(let _i=_devLog.length-1;_i>=0;_i--){ if(_devLog[_i].cargo) return _devLog[_i].cargo; } return null; })();
    const _popDelta=(p.population||0)-(p.populationBase||0);
    const _dlGrowth=(p.devLevel||0)-(p.devLevelBase||0);
    // Format helpers
    const _fmtSd=sd=>'SD '+sd.toFixed(2);
    const _fmtDelta=n=>(n>0?'+':'')+n.toLocaleString();
    const _topCargoLbl=_topCargoPair?((CARGO_LABEL[_topCargoPair[0]]||_topCargoPair[0])+' ('+_topCargoPair[1]+')'):'—';
    const _lastCargoLbl=_lastCargoType?(CARGO_LABEL[_lastCargoType]||_lastCargoType):'—';

    // Two-column layout: 5 rows × 2 cols = 10 stats
    const _statLines=[
      ['ORBITS',      _totalOrb.toLocaleString()],
      ['TRAINS',      _uniqTrains+''],
      ['DELIVERIES',  _devLog.length.toLocaleString()],
      ['DEV PTS',     Math.round(_devPts).toLocaleString()],
      ['POP Δ',  _fmtDelta(_popDelta)],
      ['DEV LVL Δ', _fmtDelta(_dlGrowth)],
      ['TOP CARGO',   _topCargoLbl],
      ['LAST CARGO',  _lastCargoLbl],
      ['FIRST',       _firstSd!=null?_fmtSd(_firstSd):'—'],
      ['LAST',        _lastSd!=null?_fmtSd(_lastSd):'—'],
    ];
    const _statY0=tcY+30, _statRowH=13;
    const _statColX=[px+14, px+14+(pw-28)/2];
    const _statColW=(pw-28)/2 - 6;
    ctx.font='9px "Exo 2",sans-serif';
    for(let _si=0;_si<_statLines.length;_si++){
      const [_lbl,_val]=_statLines[_si];
      const _row=_si%5;
      const _col=_si<5?0:1;
      const _sy=_statY0+_row*_statRowH;
      const _sx=_statColX[_col];
      ctx.fillStyle='rgba(100,145,215,0.55)';
      ctx.fillText(_lbl+':',_sx,_sy);
      const _lblW=ctx.measureText(_lbl+':').width;
      // Truncate the value if it would overflow the column
      let _valTxt=_val+'';
      const _maxVW=_statColW-_lblW-4;
      while(ctx.measureText(_valTxt).width>_maxVW&&_valTxt.length>3) _valTxt=_valTxt.slice(0,-1);
      if(_valTxt!==(_val+'')) _valTxt=_valTxt.slice(0,-1)+'…';
      ctx.fillStyle='rgba(180,220,255,0.92)';
      ctx.fillText('  '+_valTxt,_sx+_lblW,_sy);
    }

    // Faint divider between sections
    const _divY=_statY0+5*_statRowH+6;
    ctx.strokeStyle='rgba(40,90,180,0.28)'; ctx.lineWidth=0.5;
    ctx.beginPath(); ctx.moveTo(px+14,_divY); ctx.lineTo(px+pw-14,_divY); ctx.stroke();

    // ── Section 2: TRAIN VISITS (TOP 5) ────────────────────────
    const _tvHdrY=_divY+12;
    ctx.font='bold 9px Orbitron,sans-serif'; ctx.textAlign='left';
    ctx.fillStyle='rgba(100,160,255,0.7)';
    ctx.fillText('TRAIN VISITS — TOP 5',px+14,_tvHdrY);

    const visitROW=13, visitListY=_tvHdrY+6;
    if(pocE.length===0){
      ctx.font='10px "Exo 2",sans-serif'; ctx.fillStyle='rgba(70,90,130,0.6)';
      ctx.fillText('No trains have orbited this planet yet.',px+14,visitListY+11);
    } else {
      ctx.font='10px "Exo 2",sans-serif';
      const _maxRows=5;
      const _shown=Math.min(pocE.length,_maxRows);
      for(let ei=0;ei<_shown;ei++){
        const ey=visitListY+ei*visitROW+9;
        const [ti,cnt]=pocE[ei];
        const tr=trains[ti];
        const tc=tr?.color||'#4af';
        ctx.fillStyle=tc; ctx.fillRect(px+14,ey-7,7,7);
        ctx.strokeStyle='rgba(255,255,255,0.15)'; ctx.lineWidth=0.5; ctx.strokeRect(px+14,ey-7,7,7);
        ctx.fillStyle='rgba(160,200,255,0.85)'; ctx.textAlign='left';
        let tname=(tr?tr.name:'Train '+ti);
        // Truncate train name to fit; reserve ~80px for the orbit count suffix
        const _nameMaxW=pw-28-80;
        while(ctx.measureText(tname).width>_nameMaxW&&tname.length>3) tname=tname.slice(0,-1);
        ctx.fillText(tname+'  \xb7  '+cnt+' orbit'+(cnt!==1?'s':''),px+24,ey);
      }
      if(pocE.length>_maxRows){
        ctx.font='italic 9px "Exo 2",sans-serif'; ctx.fillStyle='rgba(110,145,200,0.55)';
        ctx.fillText('… and '+(pocE.length-_maxRows)+' more train'+(pocE.length-_maxRows!==1?'s':''),px+24,visitListY+_maxRows*visitROW+8);
      }
    }
  }
...
function drawGalaxy(ts,dt){
  ...
  if((sel&&(sel.type==='planet'||sel.type==='star')&&routeStops.length>=2)||routeHerePending){
    ...
    if(_ibPVis){
      ...
      if(p.hasStation) drawPlanetStation(0,GH+BAR_H/2,pDR,0,pDR,p.isAlienRelic,p.hasLargeStation||false,null,p.hasTerminal||false);
        ...
        if(_newMissionAcceptBounds){const b=_newMissionAcceptBounds; if(cp.x>=b.x&&cp.x<=b.x+b.w&&cp.y>=b.y&&cp.y<=b.y+b.h){
          ...
          if(def&&!missions.some(mx=>mx.id===def.id)){
            missions.push({id:def.id,name:def.name,imageType:def.imageType,imageKey:def.imageKey||null,
              ...
              details:(()=>{let _d=def.details||'';
                if(_d.includes('{SOURCE}')&&popupState.sourcePlanetId!=null){const _sp5=_gp(popupState.sourcePlanetId);if(_sp5)_d=_d.replace(/\{SOURCE\}/g,_sp5.name);}
                if(_d.includes('{TARGET}')&&popupState.targetPlanetId!=null){const _tp5=_gp(popupState.targetPlanetId);if(_tp5)_d=_d.replace(/\{TARGET\}/g,_tp5.name);}
                return _d;})(),reward:def.reward||null,timeLimit:def.timeLimit||null,
              ...
              visitedSnapshot:(def.id==='find_molten_ore'||def.id==='build_foundry'||def.id==='stellar_cartography')?new Set():new Set(visitedPlanetIds),ironUnlockedSnapshot:_ironCarUnlocked,
...
function _buildSaveObject(){
  ...
  // Encode current selection by stable id, since the live `sel` holds object
  // references that won't survive serialization.
  const _savedSel=(()=>{
    if(!sel) return null;
    if(sel.type==='train'){const _ti=trains.indexOf(sel.train);return _ti>=0?{type:'train',trainIdx:_ti}:null;}
    if(sel.type==='car'  ) return {type:'car', trainIdx:sel.data?.trainIdx??-1, carIdx:sel.data?.carIdx??-1};
    if(sel.type==='planet') return {type:'planet', planetId:sel.data?.id};
    if(sel.type==='star'  ) return {type:'star',   starId:sel.data?.id};
    return null;
  })();
  return {
...
function _restoreFromSave(save){
  if(!save||(save.version!==1&&save.version!==2)){ alert('Incompatible save file.'); return; }
  ...
  const planets=sg.planets.map(p=>{
    ...
    // Re-attach biome-specific procedural fields that are stripped from saves:
    //   resort/agri/oil → _b (landmasses/swatches); storm → _b (lightning bolts);
    //   any biome → _mtn (mountain ridges).
  ...
  {
    const _savedIdx=save.gameSpeedIdx;
    if(_savedIdx==null) gameSpeedIdx=SPEED_DEFAULT_IDX;
    else if(save.version===1) gameSpeedIdx=Math.min(SPEED_OPTS.length-1, _savedIdx+2);
    else gameSpeedIdx=Math.max(0,Math.min(SPEED_OPTS.length-1, _savedIdx|0));
  }
  ...
  // first car of the first player train if the saved selection can't be
  // resolved (e.g. the referenced train was scrapped pre-save somehow), or
  // null if there are no trains.
  const _selSave=save.sel;
  sel=null;
  if(_selSave){
    if(_selSave.type==='train'){
      const _t=trains[_selSave.trainIdx];
      if(_t) sel={type:'train', train:_t};
    } else if(_selSave.type==='car'){
      const _t=trains[_selSave.trainIdx];
      const _c=_t?.cars?.[_selSave.carIdx];
      if(_t&&_c) sel={type:'car', data:{trainIdx:_selSave.trainIdx, carIdx:_selSave.carIdx, car:_c}};
    } else if(_selSave.type==='planet'){
      const _p=galaxy.planets[_selSave.planetId];
      if(_p) sel={type:'planet', data:_p};
    } else if(_selSave.type==='star'){
      const _s=galaxy.stars.find(s=>s.id===_selSave.starId);
      if(_s) sel={type:'star', data:_s};
    }
  }
  if(!sel && trains[0]?.cars?.[0]){
    sel={type:'car', data:{trainIdx:0, carIdx:0, car:trains[0].cars[0]}};
  }
  // Restore tracking state. When tracking is on, the camera is kept locked to
  // the selected object's world position offset by trackingOffset. The saved
  // cam.x/y already reflects the post-tracking camera, so the lock will hold
  // immediately on first tick after restore.
  tracking=save.tracking!==undefined ? !!save.tracking : false;
  trackingOffset=save.trackingOffset ? {x:save.trackingOffset.x||0, y:save.trackingOffset.y||0} : {x:0,y:0};
...
// Legacy single-slot id (pre per-corp autosave split). Still recognized for
// backward compat so old autosaves remain loadable until the user deletes them.
const LS_AUTOSAVE_ID='autosave';
// New per-corp autosave id form: 'autosave_<safe_corp_name>'. Each corp gets
// its own slot, so a new game can't clobber the previous corp's autosave.
const LS_AUTOSAVE_PREFIX='autosave_';
function _lsAutosaveId(){ return LS_AUTOSAVE_PREFIX+_lsSafeCorp(); }
function _isAutosaveId(id){ return id===LS_AUTOSAVE_ID || (typeof id==='string' && id.indexOf(LS_AUTOSAVE_PREFIX)===0); }
...
function _autosaveToLocalStorage(){
  ...
  // Per-corp slot id, so a fresh game with a new corp name doesn't overwrite
  // the previous corp's autosave. Each corp has its own autosave slot.
  const _slotId=_lsAutosaveId();
  const _label='Autosave: '+(corpName||'Save');
  const res=_lsWriteSlot(_slotId,saveObj,_label);
...
function startGame(){
  ...
  activePopup=null; popupState={}; gameSpeedIdx=SPEED_DEFAULT_IDX; _popupCooldownUntil=0; _prevHadPopup=false; _missionTipStartMs=0; _speedTipStartMs=0; _speedTipSuppressed=false; _zoomCalloutStartMs=0; _createRouteTimerMs=0; _findOreTimerMs=0; _produceIronTimerMs=0; _steelMissionTimerMs=0; _galaxyCensusTimerMs=0; _hasZoomed=false;
    ...
    if(!activePopup&&pendingMissionIntros.length>0&&Date.now()>=_popupCooldownUntil){
      ...
      if(_nmiIdx>=0){
        ...
        if(_nmDef){
          ...
          const _tmpM={
            visitedSnapshot:(_nmDef.id==='find_molten_ore'||_nmDef.id==='build_foundry'||_nmDef.id==='stellar_cartography')?new Set():new Set(visitedPlanetIds),
          ...
          if(_allPreDone){
            ...
            if(!missions.some(mx=>mx.id===_nmDef.id)){
              missions.push({id:_nmDef.id,name:_nmDef.name,imageType:_nmDef.imageType,imageKey:_nmDef.imageKey||null,
                ...
                details:(()=>{let _d=_nmDef.details||'';
                  if(_d.includes('{SOURCE}')&&_nmiE.sourcePlanetId!=null){const _sp6=_gp(_nmiE.sourcePlanetId);if(_sp6)_d=_d.replace(/\{SOURCE\}/g,_sp6.name);}
                  if(_d.includes('{TARGET}')&&_nmiE.targetPlanetId!=null){const _tp6=_gp(_nmiE.targetPlanetId);if(_tp6)_d=_d.replace(/\{TARGET\}/g,_tp6.name);}
                  return _d;})(),reward:_nmDef.reward||null,timeLimit:_nmDef.timeLimit||null,
